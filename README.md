...
The canonical simulation parameters are the committed defaults in `sim/config.py`. The local `.env` file should contain only environment-specific values and safe overrides from `.env.example`, such as cohort percentage, NFT threshold, local deployment addresses, and controller settings. Visitors do not need to guess separate hidden market/regime parameters.
